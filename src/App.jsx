...
const SUPABASE_KEY = "sb_publishable_WTcQfalj1Mjn2LwdSYp-EA_-_fFxfV2";
  ...
  return (
    <div style={{ marginBottom:12 }}>
      ...
      <textarea value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} rows={3}
        style={{ width:"100%", border:"1.5px solid #e5e7eb", borderRadius:10, padding:"10px 12px", fontSize:13, fontFamily:"inherit", boxSizing:"border-box", outline:"none", resize:"none", background:"#fafafa" }} />
    </div>
  );
}
...
const INP = { width:"100%", border:"1.5px solid #e5e7eb", borderRadius:10, padding:"10px 12px", fontSize:14, fontFamily:"inherit", boxSizing:"border-box", outline:"none", background:"#fafafa" };
...
function DetailSheet({ student, onClose, onRecharge, onLessonClick, onShift, onTelafiDone, onMesaj, onOdeme, onDelete }) {
    ...
    <>
      <Sheet title={student.name} onClose={onClose}>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:14 }}>
          {[{ key:"takvim", label:"📅 Dersler" }, { key:"telafi", label:`🔄 Telafiler${active.length>0?" ("+active.length+")":""}` }].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{ flex:1, background:tab===t.key?"#111":"#f3f4f6", color:tab===t.key?"#fff":"#555", border:"none", borderRadius:10, padding:"9px 12px", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
              {t.label}
            </button>
          ))}
        </div>
...
function msgDersHatirlatma(student) {
  const next = student.schedule.find(l => l.status === "upcoming");
  if (!next) return "";
  return `Merhaba 👋\n\n${student.name}'nın bugün saat ${student.time}'de *Sonsuz Sanat Müzik Okulu*'nda dersi bulunmaktadır.\n\nDerse zamanında gelmenizi rica ederiz. Görüşürüz! 🎵`;
}
...
export default function App() {
  ...
  const isOdemeBekleyen = (s) => {
    ...
    if (!np) return false;
    return midday(new Date(np)).getTime() <= todayMid.getTime();
  };
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:"#f4f4f0", minHeight:"100vh" }}>
      ...
      <div style={{ background:"#111", color:"#fff", padding:"16px 20px 0" }}>
        ...
        <div style={{ maxWidth:600, margin:"0 auto", display:"flex", gap:4 }}>
          {[{key:"liste",label:"📋 Liste"},{key:"takvim",label:"📅 Takvim"}].map(t=>(
            <button key={t.key} onClick={()=>setMainTab(t.key)} style={{ flex:1, background:mainTab===t.key?"#fff":"transparent", color:mainTab===t.key?"#111":"#888", border:"none", borderRadius:"10px 10px 0 0", padding:"10px 0", fontWeight:700, fontSize:14, cursor:"pointer", fontFamily:"inherit" }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {mainTab === "liste" && (
          <div>
            {todayPayments.length > 0 && (
              <div style={{ background:"#fff7ed", border:"1.5px solid #fb923c", borderRadius:14, padding:"12px 16px", marginBottom:14 }}>
                <p style={{ margin:0, fontWeight:700, fontSize:13, color:"#c2410c" }}>💳 Bugün alınacak ödeme{todayPayments.length>1?"ler":""}:</p>
                {todayPayments.map(s=>(<p key={s.id} style={{ margin:"4px 0 0", fontSize:13, color:"#9a3412" }}>· {s.name} — <strong>{fmtMed(calcNextPayment(s.schedule))}</strong></p>))}
              </div>
            )}
            ...
            <div style={{ display:"flex", gap:6, marginBottom:14, overflowX:"auto", paddingBottom:2 }}>
              {[{key:"all",label:"Tümü"},{key:"active",label:"Aktif"},{key:"frozen",label:"Dondurulmuş"},{key:"telafi",label:"Telafililer"},{key:"odeme",label:"💳 Ödeme Bekleyen"}].map(f=>(
      ...
      {detailSt && <DetailSheet student={students.find(s=>s.id===detailSt.id)} onClose={()=>setDetailSt(null)} onRecharge={handleRecharge} onLessonClick={(st,lid)=>{ setDetailSt(null); setTimeout(()=>setActionModal({student:st,lessonId:lid}),100); }} onShift={handleShift} onTelafiDone={handleTelafiDone} onMesaj={(st)=>setMesajSt(st)} onOdeme={(st)=>setOdemeSt(st)} onDelete={handleDelete} />}
...
function buildNefes() {
  const dates = [];
  const d = nextWeekday("Salı");
  for (let i = 0; i < 4; i++) { dates.push(new Date(d).toISOString()); d.setDate(d.getDate()+7); }
  return [
    { id:"n1", date:dates[0], status:"completed", note:"" },
    { id:"n2", date:dates[1], status:"telafi",    note:"Hasta olduğunu bildirdi" },
    { id:"n3", date:dates[2], status:"telafi",    note:"Okul gezisi" },
    { id:"n4", date:dates[3], status:"upcoming",  note:"" },
  ];
}

const INIT = [
  { id:1, name:"Ahmet Yılmaz", instrument:"Davul",  day:"Pazartesi", time:"15:00", noShow:0, frozen:false, schedule:buildSchedule("Pazartesi",4), telafiRecords:[] },
  { id:2, name:"Nefes Kaya",   instrument:"Piyano", day:"Salı",      time:"16:00", noShow:0, frozen:false, schedule:buildNefes(),
    telafiRecords:[
      { id:"t1", lessonDate:buildNefes()[1].date, note:"Hasta olduğunu bildirdi", createdAt:new Date(Date.now()-10*86400000).toISOString(), expiry:expiry30(), done:false, doneAt:null },
      { id:"t2", lessonDate:buildNefes()[2].date, note:"Okul gezisi",             createdAt:new Date(Date.now()-3*86400000).toISOString(),  expiry:expiry30(), done:false, doneAt:null },
    ]
  },
  { id:3, name:"Mert Demir",   instrument:"Gitar",  day:"Cuma",      time:"17:00", noShow:0, frozen:true,  schedule:buildSchedule("Cuma",4), telafiRecords:[] },
];
...
function ActionSheet({ student, lessonId, onClose, onAction }) {
  ...
  const lesson = lessonId ? student.schedule.find(l=>l.id===lessonId) : student.schedule.find(l=>l.status==="upcoming");
  const activeTelafi = student.telafiRecords.filter(r=>!r.done).length;
  const willWarn   = activeTelafi === 4;
  const willFreeze = activeTelafi === 5;
...
function DetailSheet({ student, onClose, onRecharge, onLessonClick, onShift, onTelafiDone, onMesaj, onOdeme }) {
  const [tab, setTab] = useState("takvim");
  const [telafiSel, setTelafiSel] = useState(null);
  const [shiftSel, setShiftSel]   = useState(null);
  ...
  const bal = calcBalance(student.schedule);
  const np  = calcNextPayment(student.schedule);
  const active = student.telafiRecords.filter(r=>!r.done);
  const done   = student.telafiRecords.filter(r=>r.done);
  ...
  return (
    <>
      <Sheet title={student.name} onClose={onClose}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:12 }}>
          {[
            { label:"Kalan Ders",   val:bal,          bg:"#f9fafb", color:"#111" },
            { label:"Aktif Telafi", val:active.length, bg: active.length>=5?"#fee2e2":active.length===4?"#fffbeb":"#eff6ff", color: active.length>=5?"#dc2626":active.length===4?"#d97706":"#2563eb" },
            { label:"No-Show",      val:student.noShow, bg:"#fff1f2", color:"#e11d48" },
          ].map(s => (
            <div key={s.label} style={{ background:s.bg, borderRadius:12, padding:"12px 8px", textAlign:"center" }}>
              <p style={{ fontSize:24, fontWeight:800, color:s.color, margin:0 }}>{s.val}</p>
              <p style={{ fontSize:10, color:"#888", margin:"2px 0 0", fontWeight:600 }}>{s.label}</p>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:14 }}>
          {[
            ...
            { key:"telafi", label:`🔄 Telafiler${active.length>0?" ("+active.length+")":""}` },
          ].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{ flex:1, background:tab===t.key?"#111":"#f3f4f6", color:tab===t.key?"#fff":"#555", border:"none", borderRadius:10, padding:"9px 12px", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
              {t.label}
            </button>
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    <div style={{ cursor:clickable?"pointer":"default", flex:1 }} onClick={() => clickable && onLessonClick(student, l.id)}>
                      ...
                      <p style={{ margin:"2px 0 0", fontSize:12, color:"#888" }}>
                        {student.time}
                        {clickable && <span style={{ marginLeft:6, fontSize:11, color:"#93c5fd", fontWeight:600 }}>· işlem yap →</span>}
                      </p>
                    </div>
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      <StatusPill status={l.status} />
                      {clickable && (
                        <button onClick={() => setShiftSel(l)} title="Tarihi kaydır"
                          style={{ background:"#f3f4f6", border:"none", borderRadius:8, padding:"4px 8px", cursor:"pointer", fontSize:14, color:"#6366f1" }}>↦</button>
                      )}
                    </div>
                  </div>
                  {l.note && <div style={{ background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:8, padding:"6px 10px", marginTop:6 }}><p style={{ margin:0, fontSize:12, color:"#475569", fontStyle:"italic" }}>📝 {l.note}</p></div>}
        ...
        {tab === "telafi" && (
          <div>
            {student.telafiRecords.length === 0 && <p style={{ textAlign:"center", color:"#aaa", padding:"24px 0", fontWeight:600 }}>Aktif telafi hakkı yok</p>}
      ...
      {telafiSel && (
        <TelafiSheet record={telafiSel} studentName={student.name} onClose={() => setTelafiSel(null)}
          onDone={(id, note) => { onTelafiDone(student.id, id, note); setTelafiSel(null); }} />
      )}
      {shiftSel && (
        <ShiftSheet lesson={shiftSel} student={student} onClose={() => setShiftSel(null)}
          onShift={(lid, days) => { onShift(student.id, lid, days); setShiftSel(null); }} />
      )}
    </>
  );
}
...
function AddSheet({ onClose, onAdd }) {
  ...
  // Seçilen tarihten itibaren buildSchedule
  const previewDates = () => {
    if (!f.name) return "";
    const from = new Date(f.firstDate + "T12:00:00");
    return buildSchedule(f.day, f.count, from).map(l=>fmtShort(l.date)).join(" · ");
  ...
  return (
    <Sheet title="Yeni Öğrenci" onClose={onClose}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        <div>
          <label style={LBL}>Gün</label>
          <select style={INP} value={f.day} onChange={e=>s("day",e.target.value)}>
            {DAYS.map(d=><option key={d}>{d}</option>)}
          </select>
        </div>
        <div>
          <label style={LBL}>Saat</label>
          <select style={INP} value={f.time} onChange={e=>s("time",e.target.value)}>
      ...
      <label style={LBL}>İlk Ders Tarihi</label>
      <input style={INP} type="date" value={f.firstDate} onChange={e=>s("firstDate",e.target.value)} />
      <p style={{ fontSize:11, color:"#888", margin:"4px 0 0" }}>Mevcut öğrenciler için gerçek başlangıç tarihini gir — sistem oradan hesaplar.</p>

      {f.name && (
        <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:10, padding:"10px 12px", marginTop:12, fontSize:12, color:"#166534" }}>
          <strong>Planlanacak dersler:</strong><br />{previewDates()}
        </div>
      )}

      <div style={{ marginTop:16 }}>
        <Btn bg="#111" onClick={() => { if(f.name.trim()){ onAdd(f); onClose(); } }}>Kaydet</Btn>
      </div>
    </Sheet>
  );
}
...
function msgOdemeHatirlatma(student) {
  const np = calcNextPayment(student.schedule);
  return `Merhaba 👋\n\n${student.name}'nın mevcut ders paketi tamamlanmak üzere. Eğitimine kesintisiz devam edebilmesi için yeni paket ödemesini *${fmtMed(np)}* tarihine kadar yapmanızı rica ederiz.\n\nBilgi için bizimle iletişime geçebilirsiniz. Teşekkürler 🙏`;
}
        ...
        {msgs.map(m => (
          <div key={m.key} style={{ background:"#f9fafb", border:"1px solid #e5e7eb", borderRadius:12, overflow:"hidden" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 14px", borderBottom:"1px solid #f0f0f0" }}>
              <span style={{ fontWeight:700, fontSize:14, color:"#111" }}>{m.icon} {m.label}</span>
              <button
                onClick={() => copy(m.key, m.text)}
                style={{ background: copied===m.key ? "#10b981" : "#111", color:"#fff", border:"none", borderRadius:8, padding:"5px 14px", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit", flexShrink:0 }}
              >
                {copied===m.key ? "✓ Kopyalandı" : "Kopyala"}
              </button>
            </div>
            <div style={{ padding:"10px 14px" }}>
              <p style={{ margin:0, fontSize:12, color:"#555", lineHeight:1.6, whiteSpace:"pre-line" }}>{m.text}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {days.map((d,i) => {
          ...
          return (
            <div key={i} style={{ background:today?"#f0f9ff":"#fff", border:today?"1.5px solid #bae6fd":"1px solid #f0f0f0", borderRadius:14, overflow:"hidden" }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderBottom:lessons.length?"1px solid #f0f0f0":"none" }}>
                <div style={{ width:36, height:36, borderRadius:"50%", flexShrink:0, background:today?"#0ea5e9":"#f3f4f6", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, fontSize:15, color:today?"#fff":"#374151" }}>
                  {d.getDate()}
                </div>
                <div>
                  <p style={{ margin:0, fontWeight:700, fontSize:14, color:today?"#0369a1":"#111" }}>{d.toLocaleDateString("tr-TR",{weekday:"long"})}</p>
...
export default function App() {
  ...
  const [actionModal, setActionModal] = useState(null); // {student, lessonId}
  const [detailSt,    setDetailSt]    = useState(null);
  const [showAdd,     setShowAdd]     = useState(false);
  const [filter,      setFilter]      = useState("all");
  const [mainTab,     setMainTab]     = useState("liste");
  ...
  // ── Telafi kaydı oluştur ──
  const mkTelafi = (student, lid, note, extra={}) => {
    const lesson = lid ? student.schedule.find(l=>l.id===lid) : student.schedule.find(l=>l.status==="upcoming");
    return { id:uid(), lessonDate:lesson?.date||new Date().toISOString(), note, createdAt:new Date().toISOString(), expiry:expiry30(), done:false, doneAt:null, ...extra };
  };

  const handleAction = (sid, action, note="", lid=null) => {
    let msg = "Kaydedildi";
    setStudents(prev => prev.map(s => {
      ...
      switch(action) {
        ...
        case "telafi": {
          const rec = mkTelafi(s, lid, note||"24 saat öncesi iptal");
          const recs = [...s.telafiRecords, rec];
          const ac = recs.filter(r=>!r.done).length;
          const frozen = ac>=6 ? true : s.frozen;
          msg = ac>=6 ? "🚨 6. telafi — program otomatik donduruldu!" : ac===5 ? "⚠️ 5. telafi! Bir sonrakinde dondurulacak." : "🔄 Telafi hakkı oluşturuldu";
          return {...s, frozen, telafiRecords:recs, schedule: updLesson(s.schedule, lid, "telafi", note)};
        }
        ...
        case "lm-telafi": {
          ...
          const frozen = ac>=6 ? true : s.frozen;
          msg = ac>=6 ? "🚨 6. telafi — program otomatik donduruldu!" : ac===5 ? "⚠️ 5. telafi! Bir sonrakinde dondurulacak." : "⏱ Son dakika + Telafi kaydedildi";
          return {...s, frozen, telafiRecords:recs, schedule: updLesson(s.schedule, lid, "lastminute", note||"Son dakika iptali")};
        }
        ...
        case "noshow":
          msg = "🚫 Habersiz gelmedi — ders sayıldı";
          return {...s, noShow:s.noShow+1, schedule: updLesson(s.schedule, lid, "noshow", note||"Habersiz gelmedi")};
  ...
  const handleTelafiDone = (sid, tid, note) => {
    setStudents(p => p.map(s => s.id!==sid ? s : {
      ...s, telafiRecords: s.telafiRecords.map(r => r.id!==tid ? r : {...r, done:true, doneAt:note||fmtDate(new Date().toISOString())})
    }));
    pop("✅ Telafi yapıldı olarak işaretlendi");
  };

  const handleShift = (sid, fromLid, days) => {
  ...
  const handleRecharge = (sid, odemeDate) => {
    setStudents(p => p.map(s => {
      if (s.id!==sid) return s;
      const last = [...s.schedule].reverse().find(l=>l.status!=="upcoming");
      const from = last ? new Date(new Date(last.date).getTime()+7*86400000) : new Date();
  ...
  const handleAdd = (f) => {
    const from = new Date((f.firstDate || new Date().toISOString().split("T")[0]) + "T12:00:00");
    setStudents(p=>[...p,{ id:Date.now(), name:f.name, instrument:f.instrument, day:f.day, time:f.time, noShow:0, frozen:false, schedule:buildSchedule(f.day, f.count, from), telafiRecords:[] }]);
    pop("✅ Öğrenci eklendi");
  };
  ...
  const isOdemeBekleyen = (s) => {
    if (s.frozen) return false;
    const bal = calcBalance(s.schedule);
    if (bal === 0) return true; // ders bitti, ödeme bekleniyor
    const np = calcNextPayment(s.schedule);
    if (!np) return false;
    return midday(new Date(np)).getTime() <= todayMid.getTime();
  };
  ...
  const filtered = students.filter(s => {
    if (filter==="active")  return !s.frozen;
    if (filter==="frozen")  return s.frozen;
    if (filter==="telafi")  return s.telafiRecords.some(r=>!r.done);
    if (filter==="odeme")   return isOdemeBekleyen(s);
    return true;
  });
  ...
  const telafiWarnList = students.filter(s => s.telafiRecords.filter(r=>!r.done).length===5 && !s.frozen);
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:"#f4f4f0", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:600, margin:"0 auto", padding:"14px 14px 80px" }}>
        ...
        {/* Takvim */}
        {mainTab === "takvim" && (
          <WeekCal students={students} offset={weekOffset} setOffset={setWeekOffset} onStudentClick={setDetailSt} />
        )}
          ...
          <div>
            ...
            {todayPayments.length > 0 && (
              <div style={{ background:"#fff7ed", border:"1.5px solid #fb923c", borderRadius:14, padding:"12px 16px", marginBottom:14 }}>
                <p style={{ margin:0, fontWeight:700, fontSize:13, color:"#c2410c" }}>💳 Bugün alınacak ödeme{todayPayments.length>1?"ler":""}:</p>
                {todayPayments.map(s=>(
                  <p key={s.id} style={{ margin:"4px 0 0", fontSize:13, color:"#9a3412" }}>· {s.name} — <strong>{fmtMed(calcNextPayment(s.schedule))}</strong></p>
                ))}
              </div>
            )}
            ...
            {telafiWarnList.length > 0 && (
              <div style={{ background:"#fffbeb", border:"1.5px solid #fcd34d", borderRadius:14, padding:"12px 16px", marginBottom:14 }}>
                <p style={{ margin:0, fontWeight:700, fontSize:13, color:"#92400e" }}>⚠️ Telafi limitine yaklaşan öğrenci{telafiWarnList.length>1?"ler":""}:</p>
                {telafiWarnList.map(s=>(
                  <p key={s.id} style={{ margin:"4px 0 0", fontSize:13, color:"#78350f" }}>· {s.name} — <strong>5/6 telafi</strong></p>
                ))}
              </div>
            )}
            ...
            <div style={{ display:"flex", gap:6, marginBottom:14, overflowX:"auto", paddingBottom:2 }}>
              {[
                ...
                {key:"telafi", label:"Telafililer"},
                {key:"odeme",  label:"💳 Ödeme Bekleyen"},
              ].map(f=>(
                <button key={f.key} onClick={()=>setFilter(f.key)} style={{ flexShrink:0, background:filter===f.key?"#111":"#fff", color:filter===f.key?"#fff":"#555", border:"none", borderRadius:20, padding:"7px 14px", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 1px 3px rgba(0,0,0,.06)" }}>
                  {f.label}
                </button>
              ))}
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {filtered.map(s => {
                const bal  = calcBalance(s.schedule);
                const np   = calcNextPayment(s.schedule);
                const ac   = s.telafiRecords.filter(r=>!r.done).length;
                const warn = ac===5 && !s.frozen;
                const nextL = s.schedule.find(l=>l.status==="upcoming");
                const payDue = isOdemeBekleyen(s);
                  ...
                  <div key={s.id} style={{ background:s.frozen?"#f0f9ff":"#fff", borderRadius:16, padding:"14px 16px", boxShadow:"0 1px 4px rgba(0,0,0,.07)", border:warn?"1.5px solid #fcd34d":payDue?"1.5px solid #fb923c":s.frozen?"1.5px solid #bfdbfe":"1.5px solid transparent" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div style={{ flex:1, cursor:"pointer" }} onClick={()=>setDetailSt(s)}>
                        ...
                        <p style={{ fontSize:12, color:"#999", margin:"3px 0 6px", fontWeight:500 }}>{s.instrument} · {s.day} {s.time}</p>
                        {nextL && (
                          <p style={{ fontSize:12, color:"#0369a1", fontWeight:600, margin:"0 0 6px", background:"#f0f9ff", display:"inline-block", borderRadius:6, padding:"2px 8px" }}>
                            📅 {fmtDate(nextL.date)}
                          </p>
                        )}
                        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                        ...
                        {ac>0 && <div style={{ marginTop:4 }}><span style={{ fontSize:12, color:ac>=5?"#d97706":"#2563eb" }}>🔄 <strong>{ac}/6</strong> aktif telafi</span></div>}
                        {s.noShow>0 && <div><span style={{ fontSize:12, color:"#dc2626" }}>🚫 <strong>{s.noShow}</strong> no-show</span></div>}
                      </div>
                      <button onClick={()=>setActionModal({student:s,lessonId:null})} style={{ background:s.frozen?"#e0f2fe":"#111", color:s.frozen?"#0369a1":"#fff", border:"none", borderRadius:10, padding:"8px 14px", fontSize:13, fontWeight:700, cursor:"pointer", marginLeft:10, flexShrink:0, fontFamily:"inherit" }}>
                        İşlem
                      </button>
                      <button onClick={()=>setMesajSt(s)} style={{ background:"#dcfce7", color:"#166534", border:"none", borderRadius:10, padding:"8px 10px", fontSize:16, cursor:"pointer", marginLeft:6, flexShrink:0 }}>
                        💬
                      </button>
                    </div>
                  </div>
              ...
              {filtered.length===0 && (
                <div style={{ textAlign:"center", padding:"48px 20px", color:"#bbb" }}>
                  <p style={{ fontSize:36 }}>🎵</p>
                  <p style={{ fontWeight:600, color:"#aaa" }}>Bu filtrede öğrenci yok</p>
                </div>
              )}
            </div>
          </div>
      ...
      {actionModal && (
        <ActionSheet
          student={students.find(s=>s.id===actionModal.student.id)}
          lessonId={actionModal.lessonId}
          onClose={()=>setActionModal(null)}
          onAction={(a,n,l)=>handleAction(actionModal.student.id,a,n,l)}
        />
      )}
      {detailSt && (
        <DetailSheet
          student={students.find(s=>s.id===detailSt.id)}
          onClose={()=>setDetailSt(null)}
          onRecharge={handleRecharge}
          onLessonClick={(st,lid)=>{ setDetailSt(null); setTimeout(()=>setActionModal({student:st,lessonId:lid}),100); }}
          onShift={handleShift}
          onTelafiDone={handleTelafiDone}
          onMesaj={(st)=>{ setMesajSt(st); }}
          onOdeme={(st)=>{ setOdemeSt(st); }}
        />
      )}
      {showAdd && <AddSheet onClose={()=>setShowAdd(false)} onAdd={handleAdd} />}

      {mesajSt && (
        <MesajSheet student={mesajSt} onClose={()=>setMesajSt(null)} />
      )}

      {odemeSt && (
        <OdemeSheet
          student={odemeSt}
          onClose={()=>setOdemeSt(null)}
          onOdemeAl={handleRecharge}
          onMesajGonder={(st)=>{ setMesajSt(st); }}
        />
      )}
